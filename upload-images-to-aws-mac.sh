...
echo "DEFAULT_BUCKET: $DEFAULT_BUCKET"  # add this line
echo "BUCKETNAME: $BUCKETNAME"          # add this line
echo "ISO: $ISO"                        # add this line
echo "PKG: $PKG"                        # add this line
...
cmlpkg=$(find "$(pwd)" -name "$PKG" | sort | tail -1)
echo "pkg: $PKG"
echo "cmlpkg: $cmlpkg"
if [ -n "$cmlpkg" ]; then
    echo $cmlpkg
...
for imagedef in $selection; do
    ...
    defname=$(sed -nE '/^node_definition_id:/s/^.*:[[:space:]]*([^[:space:]]+)$/\1/p' $fullpath)
    echo "fullpath: $fullpath"    # add this line
    echo "defname: $defname"      # add this line
    nodedefs_keys+=("$defname")
    nodedefs_values+=("1")
done

echo "nodedefs_keys: ${nodedefs_keys[@]}"   # add this line
echo "nodedefs_values: ${nodedefs_values[@]}"   # add this line

#for imagedef in $selection; do
#    fullpath=$(find $ISO -name $imagedef)
#    #defname=$(sed -nE '/^node_definition/s/^.*:(\s+)?(\S+)$/\2/p' $fullpath)
#    #defname=$(sed -nE '/^node_definition =/s/^.*"(\S+)"$/\1/p' $fullpath)   
#    #defname=$(sed -nE '/^node_definition_id:/s/^.*:(\s+)?(\S+)$/\1/p' $fullpath)
#    defname=$(sed -nE '/^node_definition_id:/s/^.*:[[:space:]]*([^[:space:]]+)$/\1/p' $fullpath)
#    echo "fullpath: $fullpath"    # add this line
#    echo "defname: $defname"      # add this line
#    nodedefs[$defname]="1"
...
#echo "nodedefs: ${nodedefs[@]}"   # add this line
...
if [ -n "$cmlpkg" ]; then
    dialog --progressbox "Upload software package to bucket" 20 70 < <(
        ...
        echo "cmlpkg: $cmlpkg"
        echo "tmpdir: $tmpdir"
        tar tf $cmlpkg | grep 'cml2.*.deb' | xargs -n 1 tar xf $cmlpkg
